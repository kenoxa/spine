...
case "$tier" in
    ...
    *)        _envoy_val="${SPINE_ENVOY_STANDARD_OPENCODE:-${SPINE_ENVOY_OPENCODE:-$_tier_model:$_tier_effort}}" ;;
esac
model="${_envoy_val%%:*}"
effort="${_envoy_val#*:}"
...
if [ -n "$fallback_for" ]; then
    _meta_provider="OpenCode (fallback for $fallback_for)"
    _meta_fallback_note="Note: $fallback_for was unavailable. This response was generated by OpenCode as fallback."
fi
