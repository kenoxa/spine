...
| `vertical-slice` | Feature builds, greenfield, spec-driven work | Break implementation into thin end-to-end slices spanning all layers; each slice independently demoable | "new feature", "greenfield", "spec", "tracer bullet", "vertical slice", "from scratch" |
| `architecture-depth` | Module boundaries, interface design, dependency classification | Evaluate module depth ratios; classify dependencies; apply Design It Twice for interface decisions | "deep module", "shallow module", "module depth", "design it twice", "reduce coupling" |
...
| Advisory angle | Overlapping lens |
|----------------|-----------------|
| `rigorous` consultant | `concurrency` (when race conditions are the sole concern) |
| `rigorous` consultant | `migration` (when backward compat is the sole concern) |
| `creative` consultant | `architecture-depth` (when interface design is the sole concern) |
| navigator | `dependency` (when blast radius is the sole external research question) |
