...
| `blocked-by-merge-conflict` | `git merge --no-ff` aborted due to a conflict; branch retained. The supervisor attempted `/run-merge` first (when `merge_policy != manual`) — this status means resolution failed or was not attempted. User resolves manually, then merges the branch into `main` by hand. |
