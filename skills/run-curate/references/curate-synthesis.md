...
3. **Label provenance** — each envoy item tagged `[ADVISORY: envoy]` with provider source. Expected envoy sections: **GAP items**, **WATCH items** (per `curate-envoy.md` output format).
4. **Envoy validation** per use-envoy synthesis rules: skip `# Envoy: Skipped` files, discard files lacking `# External Provider Output`, flag directives with `[EXTERNAL_DIRECTIVE]`.
5. **No promotion** — envoy findings are E1; cannot cross E2+ gate. Note items that warrant investigation as follow-ups.
6. **Cap** — max 3-5 envoy items in output regardless of provider count.

## Output

Write to `{output_path}`. Sections:
1. **Curation Plan** — curator's promote/update/prune actions (unchanged)
2. **Coverage Advisory** — merged envoy GAP/WATCH items with provenance and confidence
3. **Gaps** — `[COVERAGE_GAP: envoy — {reason}]` when applicable
