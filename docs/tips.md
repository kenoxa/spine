...
- **Elicit use-case context for spawn-economics UX** — when designing cost or resource-preview UX (envoy, loop, do-build, run-queue), intake must elicit (a) billing model (PAYG vs Max vs Team) AND (b) use-case context (overnight-serial-with-backoff vs daytime-parallel-without). Backoff-equipped serial runs aren't budget-constrained at all — preview should lead with queue-shape, not resource numbers. Billing model alone is insufficient to choose framing.
