...
- Re-aggregate Claude session telemetry 30d after 2026-05-26 (post-`parse_claude.py` Skill-call repair from spec 2622 Slice 0) and re-run `docs/specs/2622-catalog-audit/report.md` heuristic against model-side invocation data. Only after this can DEFER-DELETE verdicts ripen into DELETE. Step 1 in spec 2622-catalog-audit §5.
- Path-resolution lint for spec/runbook cross-references — surfaced by frame Q6: recent doc-drift (bdb4824 missing files, f37ba33 stale ref) is mechanical, not semantic. Lightweight static check that every backticked path in `docs/specs/**/*.md` resolves on disk would catch both classes.
